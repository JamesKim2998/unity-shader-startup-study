Shader "Custom/BlinnPhoneSpecular" {
	Properties {
	...
	SubShader {
		...
		float4 LightingTest(SurfaceOutput s, float3 lightDir, float3 viewDir, float atten) {
		    ...
		    float3 specularColor = _SpecCol * pow(specular, _SpecPower) * s.Gloss;

		    // Rim term.
		    float rimByView = 1 - abs(dot(viewDir, s.Normal));
		    float rimByLight = 1 - abs(dot(lightDir, s.Normal));
		    float rimFinal = pow(rimByLight * rimByView, 3);
